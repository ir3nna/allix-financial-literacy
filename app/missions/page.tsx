    ...
    <Card className="mx-auto w-full max-w-3xl">
      <CardContent className="pt-6">
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-allianz">
          <Zap size={14} /> Дневно предизвикателство · +{q.bonusXp} XP при верен отговор
        </div>
    ...
    return (
      <Card className="mx-auto w-full max-w-3xl">
        <CardContent className="flex flex-col gap-4 pt-6">
          <div className="flex justify-center"><Thiing name="wallet" size={80} /></div>
    ...
    <Card className="mx-auto w-full max-w-3xl">
      <CardContent className="pt-6">
        <div className="flex items-center gap-3">
          <Thiing name="wallet" size={44} />
          <div>
            <h2 className="text-lg font-extrabold">Семейните разходи днес</h2>
            <p className="text-sm font-medium text-muted">
              Питай вкъщи и запиши поне 5 разхода — от закуската до тока.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-3">
          <Button variant="outline" className="flex-1" onClick={onBack}><ChevronLeft size={16} /> Назад</Button>
          <Button className="flex-1" disabled={validRows.length < 5} onClick={finish}>
...
function SavingsMission({ onBack }: { onBack: () => void }) {
  ...
  if (finished) {
    return (
      <Card className="mx-auto w-full max-w-3xl">
        <CardContent className="flex flex-col gap-4 pt-6">
          <div className="flex justify-center"><Thiing name="piggy-bank" size={80} /></div>
  ...
  return (
    <Card className="mx-auto w-full max-w-3xl">
      <CardContent className="pt-6">
        <div className="flex items-center gap-3">
          <Thiing name="piggy-bank" size={44} />
          <div>
            <h2 className="text-lg font-extrabold">Три начина за спестяване</h2>
            <p className="text-sm font-medium text-muted">Избери 3 тактики, които реално ще пробваш този месец.</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-3">
          <Button variant="outline" className="flex-1" onClick={onBack}><ChevronLeft size={16} /> Назад</Button>
          <Button className="flex-1" disabled={picked.length !== 3} onClick={finish}>
...
function TripMission({ onBack }: { onBack: () => void }) {
  ...
  if (finished) {
    return (
      <Card className="mx-auto w-full max-w-3xl">
        <CardContent className="flex flex-col gap-4 pt-6">
          <div className="flex justify-center"><Thiing name="map" size={80} /></div>
  ...
  return (
    <Card className="mx-auto w-full max-w-3xl">
      <CardContent className="pt-6">
        <div className="flex items-center gap-3">
          <Thiing name="map" size={44} />
          <div>
            <h2 className="text-lg font-extrabold">Бюджет за училищната екскурзия</h2>
            <p className="text-sm font-medium text-muted">
              Имаш {TRIP_BUDGET} € Разпредели ги по пера — резервът е задължителен (мин. 10 €).
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-3">
          <Button variant="outline" className="flex-1" onClick={onBack}><ChevronLeft size={16} /> Назад</Button>
          <Button className="flex-1" disabled={!rulesOk} onClick={finish}>
...
export default function MissionsPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="text-2xl font-extrabold md:text-3xl">Мисии</h1>
